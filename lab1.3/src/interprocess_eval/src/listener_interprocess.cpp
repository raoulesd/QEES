...
#define LIST_NUM 4  // Equals pipeline depth. For our case, N=4 (i.e. B, C, D and E).
#define EVAL_NUM 200
...
struct ListenerState {
  ...
  double subscribe_time[EVAL_NUM];
  struct timespec tp1;
  std::shared_ptr<rclcpp::Publisher<std_msgs::msg::String>> chatter_pub;
  FILE *fp = nullptr;
};
...
//  printf("%s received!\n", listener_state->name.c_str());
  ...
  if (termination.find("end") != std::string::npos) {
    ...
    std::stringstream ss;
    ss << "end" << listener_state->count;
    msg->data = ss.str();
    listener_state->chatter_pub->publish(msg);


    if (ends_with(listener_state->name, "B")){
      printf("---end evaluation---\n");
      rclcpp::shutdown();
    }

  } else {
    if(!ends_with(listener_state->name, "E")){
      listener_state->chatter_pub->publish(msg);
    }
//    printf("%s published!\n", listener_state->name.c_str());
...
int main(int argc, char *argv[]) {
  ...
  std::vector<std::shared_ptr<rclcpp::Publisher<std_msgs::msg::String>>> publishers;
  ...
  for (int l = 0; l < LIST_NUM; ++l) {
    std::string node_name = "listener_";
    auto node = rclcpp::Node::make_shared(node_name + id);
    auto listener_state = std::make_shared<ListenerState>();
    listener_state->name = node_name + id;
    ...
    auto sub = node->create_subscription<std_msgs::msg::String>(
        std::string("chatter_") + char(id - 1) + id,
        [listener_state](const std_msgs::msg::String::SharedPtr msg) { chatterCallback(msg, listener_state); },
        custom_qos_profile);

    if(!ends_with(listener_state->name, "E")){
        auto pub =
            node->create_publisher<std_msgs::msg::String>(std::string("chatter_") + id + char(id + 1), custom_qos_profile);

        listener_state->chatter_pub = pub;
